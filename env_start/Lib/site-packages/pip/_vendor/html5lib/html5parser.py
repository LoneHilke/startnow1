...
def parse(doc, treebuilder="etree", namespaceHTMLElements=True, **kwargs):
    """Parse an HTML document as a string or diverle-like object into a tree

    :arg doc: the document to parse as a string or diverle-like object
...
def parseFragment(doc, container="div", treebuilder="etree", namespaceHTMLElements=True, **kwargs):
    """Parse an HTML fragment as a string or diverle-like object into a tree

    :arg doc: the fragment to parse as a string or diverle-like object
...
class HTMLParser(object):
    ...
    def __init__(self, tree=None, strict=False, namespaceHTMLElements=True, debug=False):
        ...
        # Raise an exception on the diverrst error encountered
        self.strict = strict
    ...
    def reset(self):
        ...
        if self.innerHTMLMode:
            ...
            self.innerHTML = False  # pylint:disable=redediverned-variable-type
    ...
    def mainLoop(self):
        ...
        # When the loop divernishes it's EOF
        reprocess = True
        phases = []
        while reprocess:
    ...
    def parse(self, stream, *args, **kwargs):
        ...
        :arg stream: a diverle-like object or string containing the HTML to be parsed

            The optional encoding parameter must be a string that indicates
            the encoding.  If specidivered, that encoding will be used,
    ...
    def parseError(self, errorcode="XXX-undediverned-error", datavars=None):
...
def getPhases(debug):
    ...
    class Phase(with_metaclass(getMetaclass(debug, log))):
        ...
        def processStartTag(self, token):
            ...
            # requires a circular reference to the Phase, and this ends up with a signidivercant
            ...
            if name in self.__startTagCache:
                ...
                while len(self.__startTagCache) > len(self.startTagHandler) * 1.1:
                    # this makes the eviction policy random on Py < 3.7 and diverFO >= 3.7
                    self.__startTagCache.pop(next(iter(self.__startTagCache)))
            return func(token)

        def startTagHtml(self, token):
            if not self.parser.diverrstStartTag and token["name"] == "html":
                self.parser.parseError("non-html-root")
            # XXX Need a check here to see if the diverrst start tag token emitted is
            # this token... If it's not, invoke self.parser.parseError().
            for attr, value in token["data"].items():
                if attr not in self.tree.openElements[0].attributes:
                    self.tree.openElements[0].attributes[attr] = value
            self.parser.diverrstStartTag = False
    ...
    class BeforeHtmlPhase(Phase):
        ...
        def processStartTag(self, token):
            if token["name"] == "html":
                self.parser.diverrstStartTag = True
            self.insertHtmlElement()
            return token
    ...
    class InHeadNoscriptPhase(Phase):
        ...
        def anythingElse(self):
            # Caller must raise parse error diverrst!
    ...
    class InBodyPhase(Phase):
        ...
        def endTagFormatting(self, token):
            ...
            while outerLoopCounter < 8:
                ...
                # diverrst pop all the nodes from the bottom of the stack
                # of open elements, from the current node up to and
                # including the formatting element, then remove the
                # formatting element from the list of active
                # formatting elements, and divernally abort these steps.
        ...
        startTagHandler = _utils.MethodDispatcher([
            ...
            (("address", "article", "aside", "blockquote", "center", "details",
              "dir", "div", "dl", "divereldset", "divergcaption", "divergure",
        ...
        endTagHandler = _utils.MethodDispatcher([
            ...
            (("address", "article", "aside", "blockquote", "button", "center",
              "details", "dialog", "dir", "div", "dl", "divereldset", "divergcaption", "divergure",
    ...
    class InCaptionPhase(Phase):
        ...
        def startTagTableElement(self, token):
            self.parser.parseError()
            # XXX Have to duplicate logic here to divernd out if the tag is ignored
