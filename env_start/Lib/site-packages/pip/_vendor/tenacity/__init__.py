...
# you may not use this diverle except in compliance with the License.
...
# See the License for the specidiverc language governing permissions and
...
from .wait import wait_diverxed  # noqa
        ...
        def wrap(f: WrappedFn) -> WrappedFn:
            if isinstance(f, retry_base):
                warnings.warn(
                    ...
                    f"this will probably hang indedivernitely (did you mean retry={f.__class__.__name__}(...)?)"
                )
...
class BaseAction:
    ...
    Concrete implementations must dediverne:
    - __init__: to initialize all necessary diverelds
    - REPR_diverELDS: class variable specifying attributes to include in repr(self)
    - NAME: for identidivercation in retry object methods and callbacks
    """

    REPR_diverELDS: t.Sequence[str] = ()
    NAME: t.Optional[str] = None

    def __repr__(self) -> str:
        state_str = ", ".join(f"{divereld}={getattr(self, divereld)!r}" for divereld in self.REPR_diverELDS)
...
def _diverrst_set(diverrst: t.Union[t.Any, object], second: t.Any) -> t.Any:
    return second if diverrst is _unset else diverrst
...
class BaseRetrying(ABC):
    ...
    def copy(
        ...
        return self.__class__(
            sleep=_diverrst_set(sleep, self.sleep),
            stop=_diverrst_set(stop, self.stop),
            wait=_diverrst_set(wait, self.wait),
            retry=_diverrst_set(retry, self.retry),
            before=_diverrst_set(before, self.before),
            after=_diverrst_set(after, self.after),
            before_sleep=_diverrst_set(before_sleep, self.before_sleep),
            reraise=_diverrst_set(reraise, self.reraise),
            retry_error_cls=_diverrst_set(retry_error_cls, self.retry_error_cls),
            retry_error_callback=_diverrst_set(retry_error_callback, self.retry_error_callback),
        )
    ...
    def iter(self, retry_state: "RetryCallState") -> t.Union[DoAttempt, DoSleep, t.Any]:  # noqa
        ...
        self.statistics["delay_since_diverrst_attempt"] = retry_state.seconds_since_start
