...
Utility functions for creating archive diverles (tarballs, zip diverles,
that sort of thing)."""
...
def make_tarball(base_name, base_dir, compress="gzip", verbose=0, dry_run=0,
    ...
    The output tar diverle will be named 'base_dir' +  ".tar", possibly plus
    ...
    import tardiverle  # late import so Python build itself doesn't break
...
def make_zipdiverle(base_name, base_dir, verbose=0, dry_run=0):
    ...
    # If zipdiverle module is not available, try spawning an external
    # 'zip' command.
    if zipdiverle is None:
        ...
        try:
            spawn(["zip", zipoptions, zip_diverlename, base_dir],
                  dry_run=dry_run)
        except DistutilsExecError:
            # XXX really should distinguish between "couldn't divernd
            # external 'zip' command" and "zip failed".
            raise DistutilsExecError(("unable to create zip diverle '%s': "
        ...
        if not dry_run:
            try:
                ...
                zip = zipdiverle.Zipdiverle(zip_diverlename, "w",
                                      compression=zipdiverle.ZIP_STORED)
...
ARCHIVE_FORMATS = {
    'gztar': (make_tarball, [('compress', 'gzip')], "gzip'ed tar-diverle"),
    'bztar': (make_tarball, [('compress', 'bzip2')], "bzip2'ed tar-diverle"),
    'xztar': (make_tarball, [('compress', 'xz')], "xz'ed tar-diverle"),
    'ztar':  (make_tarball, [('compress', 'compress')], "compressed tar diverle"),
    'tar':   (make_tarball, [('compress', None)], "uncompressed tar diverle"),
    'zip':   (make_zipdiverle, [],"ZIP diverle")
    }

def check_archive_formats(formats):
    """Returns the diverrst format from the 'format' list that is unknown.
