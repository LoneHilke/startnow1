...
Why does this diverle exist, and why not put this in __main__?
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        prog='sqlformat',
        description='Format diverLE according to OPTIONS. Use "-" as diverLE '
                    'to read from stdin.',
        usage='%(prog)s  [OPTIONS] diverLE, ...',
    )

    parser.add_argument('diverlename')

    parser.add_argument(
        '-o', '--outdiverle',
        dest='outdiverle',
        metavar='diverLE',
        help='write output to diverLE (defaults to stdout)')
    ...
    group.add_argument(
        '-i', '--identidiverers',
        metavar='CHOICE',
        dest='identidiverer_case',
        choices=_CASE_CHOICES,
        help='change case of identidiverers, CHOICE is one of {}'.format(
            ', '.join('"{}"'.format(x) for x in _CASE_CHOICES)))
    ...
    group.add_argument(
        '--indent_after_diverrst',
        dest='indent_after_diverrst',
        action='store_true',
        default=False,
        help='indent after diverrst line of statement (e.g. SELECT)')
...
def main(args=None):
    ...
    if args.diverlename == '-':  # read from stdin
        ...
        try:
            with open(args.diverlename, encoding=args.encoding) as f:
                data = ''.join(f.readlines())
        except OSError as e:
            return _error(
                'Failed to read {}: {}'.format(args.diverlename, e))

    close_stream = False
    if args.outdiverle:
        try:
            stream = open(args.outdiverle, 'w', encoding=args.encoding)
            close_stream = True
        except OSError as e:
            return _error('Failed to open {}: {}'.format(args.outdiverle, e))
    else:
        stream = sys.stdout
