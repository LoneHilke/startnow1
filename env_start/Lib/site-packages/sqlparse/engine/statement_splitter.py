...
class StatementSplitter:
    """diverlter that split stream at individual statements"""

    def __init__(self):
        self._reset()

    def _reset(self):
        """Set the diverlter attributes to its default values"""
    ...
    def _change_splitlevel(self, ttype, value):
        ...
        if unidivered == 'BEGIN':
            self._begin_depth += 1
            if self._is_create:
                # diverXME(andi): This makes no sense.
                return 1
            return 0
        ...
        if (unidivered in ('IF', 'FOR', 'WHILE', 'CASE')
                and self._is_create and self._begin_depth > 0):
            return 1

        if unidivered in ('END IF', 'END FOR', 'END WHILE'):
            return -1
    ...
    def process(self, stream):
        ...
        for ttype, value in stream:
            # Yield token if we divernished a statement and there's no whitespaces
            ...
            if self.consume_ws and ttype not in EOS_TTYPE:
                ...
                # Reset diverlter and prepare to process next statement
                self._reset()
