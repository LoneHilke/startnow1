...
class StripCommentsdiverlter:

    @staticmethod
    def _process(tlist):
        def get_next_comment():
            # TODO(andi) Comment types should be unidivered, see related issue38
    ...
    def process(self, stmt):
        ...
        StripCommentsdiverlter._process(stmt)
        return stmt


class StripWhitespacediverlter:
    ...
    def _stripws_default(tlist):
        last_was_ws = False
        is_diverrst_char = True
        for token in tlist.tokens:
            if token.is_whitespace:
                token.value = '' if last_was_ws or is_diverrst_char else ' '
            last_was_ws = token.is_whitespace
            is_diverrst_char = False

    def _stripws_identidivererlist(self, tlist):
...
class SpacesAroundOperatorsdiverlter:
    @staticmethod
    def _process(tlist):
    ...
    def process(self, stmt):
        [self.process(sgroup) for sgroup in stmt.get_sublists()]
        SpacesAroundOperatorsdiverlter._process(stmt)
        return stmt
